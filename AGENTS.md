...
`docs/` 只写 ADP 与 phase 文档；不要在 `docs/` 中新增普通说明、会议记录、临时分析、教程、草稿或实现笔记。
...
- phase 文档也放在 `docs/` 中。
- phase 文档只记录阶段目标、范围、验收标准、退出条件和当前状态。
- phase 文档不能替代 ADP；如果阶段中出现满足 ADP 三条件的决策，必须另写 ADP。
...
- UI/UX 实现必须严格按照产品设计原型 `docs/ui-ux/prototypes`
