# PRD — Loupe：把页面上的“指一下”变成 Agent 能精确读取的工程任务
...
| 本地 daemon | `@loupe/server` | 监听 `127.0.0.1:7373`。暴露 authenticated `/v1/marks*` 与 `/mcp`，维护 `~/.loupe/marks.json` 镜像 |
...
3. **Locator bundle**
   - 多证据采集：selector、stable attrs、role/name、text hash、parent chain、geometry、shadow path 等。
...
function resolve(locator: Locator, root: Document | ShadowRoot | Element): ResolveResult;
```
...
type Locator = {
  ...
  evidence: {
    ...
    nth_path: string;
    parent_chain: { tag: string; role?: string; stable_attr?: string; stable_class?: string }[];
    shadow_path?: string[];
    geometry?: { x: number; y: number; width: number; height: number; viewport_width: number; viewport_height: number; dpr: number };
  };
};
```
...
- Schema migration 只以 `schema_version` 判断；不能靠字段猜测版本。
...
3. 若连接失败，则启动 `@loupe/server serve --port 7373 --daemon`。
...
type AgentMark = {
  ...
  lifecycle: { task_status: 'open' | 'resolved' | 'archived'; created_at: string; updated_at: string };
};
```
...
Claude 插件采用 **stdio MCP proxy**，proxy 读取 `~/.loupe/token` 并转发到 daemon `/mcp`。这样 `.mcp.json` 不需要明文 token，同时 daemon 仍提供 MCP-over-HTTP 给 generic clients。
...
```json
{
  "mcpServers": {
    "loupe": {
      "type": "stdio",
      "command": "npx",
      "args": ["-y", "@loupe/claude-plugin", "mcp-proxy", "--url", "http://127.0.0.1:7373/mcp"],
      "timeout": 5000
    }
  }
}
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx",
            "args": ["-y", "@loupe/server", "ensure", "--port", "7373"],
            "timeout": 8,
            "async": true
          }
        ]
...
`@loupe/server ensure` 的 contract：请求 `/health`；若不是 Loupe daemon 则启动；写 `~/.loupe/server.json`；确保 `~/.loupe/token` 存在。
...
tools: ["mcp__loupe__list_marks", "mcp__loupe__get_mark", "mcp__loupe__resolve_mark", "Read", "Edit", "Grep", "Glob"]
---
  ...
  "plugins": [
    {
      ...
      "source": {
        "source": "git-subdir",
        "url": "https://github.com/<org>/loupe",
...
- **Drift/lost correctness**：目标移动、文本变化、节点删除、route 切换后状态是否正确。
...
| 里程碑 | 交付 | 验收 |
|---|---|---|
| M0 Schema / daemon health / empty MCP | shared snake_case schema、project/session scope、daemon `/health`、token/server files、MCP empty `list_marks` | Claude MCP proxy 可连接；无 token 访问 `/mcp` 失败；scoped `list_marks` 返回空数组 |
| M1 Locator robustness | locator capture model、`resolve()`、candidate scoring、ambiguity downgrade、robustness suite | Top-1 / false-resolved / ambiguity / drift-lost 指标可自动输出；阈值可校准 |
...
| M3 Daemon persistence / project-scoped MCP | authenticated `/v1/marks*`、atomic `marks.json`、MCP four tools、AgentMark payload | Agent scoped read/get/resolve/delete contract 通过；bare-id cross-project mutation 被拒绝 |
| M4 Drift recovery / sync / extension E2E | route epoch recovery、DOM quiet window、sync retry、Playwright MV3 E2E | rerender/route/service-worker/daemon offline 场景通过；无 stale route pin commit |
| M5 Launch polish / marketplace | onboarding branches、visual polish、CLI status/init/logs、official marketplace package | Marketplace schema 验证；用户可从 marketplace 安装并完成首个 mark |
...
| Drift/lost correctness | 扰动后状态分类正确比例 | ≥ 95% |
