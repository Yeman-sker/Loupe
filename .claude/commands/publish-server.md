...
Prepare a release of `@loupe-server/server`. Do **not** run `npm publish`.

Run the shared release runner:

```bash
node .claude/hooks/release-package.mjs server $ARGUMENTS
```
...
- The runner must build, test, dry-run pack, validate package contents, validate `bin`, and stop before publish.
...
```bash
node .claude/hooks/release-package.mjs mark-published server
```
