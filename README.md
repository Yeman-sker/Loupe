...
- [CLI 参考（`@loupe/server`）](#cli-参考loupeserver)
...
Loupe 是一个 pnpm monorepo，包含三个交付组件 + 一个共享包：

| 组件 | 包名 | 职责 |
| ---- | ---- | ---- |
| 浏览器扩展 | `@loupe/extension` | Chrome MV3。picker、composer、locator/context 采集、minimal pin overlay、`chrome.storage.local` 本地优先存储 |
| 本地 daemon | `@loupe/server` | 监听 `127.0.0.1:7373`，暴露 `/health`、authenticated `/v1/marks*` 与 `/mcp`，维护 `~/.loupe/marks.json` 镜像。同时提供 `loupe` CLI |
| Claude 插件 | `@loupe/claude-plugin` | 启动 daemon（SessionStart hook）、stdio MCP proxy、`/loupe:marks` 命令与 `mark-resolver` agent |
| 共享 schema/逻辑 | `@loupe/shared` | snake_case wire schema、`resolve()` locator 评分、route-recovery 状态机 |
...
Chrome 扩展 ──(本地优先写 chrome.storage.local)──┐
   ...
   └──────────────── POST /v1/marks ──────────────▶  本地 daemon (127.0.0.1:7373)
                                                       ...
                                                       │
Agent / Claude ──(stdio MCP proxy 注入 token)──▶ POST /mcp ──┘
...
> 说明：`@loupe/server`、`@loupe/extension`、`@loupe/shared` 直接以 TypeScript 源码 + `tsx` loader 运行，无需单独 build；只有 `@loupe/claude-plugin` 会 `tsc` 编译到 `dist/`（已随仓库提供）。扩展的运行时脚本 `src/content.js` / `src/background.js` 是直接被 Chrome 加载的手写 MV3 脚本，同名 `.ts` 文件用于类型检查与测试。
...
**B. 已发布包（`@loupe/server` 提供 `loupe` / `loupe-server` 两个 bin）**

```bash
npx -y @loupe/server ensure --port 7373   # 没在跑就拉起，已在跑就复用
npx -y @loupe/server status
```

**C. 交给 Claude 插件自动拉起** —— 插件的 `SessionStart` hook 会执行 `@loupe/server ensure --port 7373`（见 [`packages/claude-plugin/hooks/hooks.json`](./packages/claude-plugin/hooks/hooks.json)），无需手动启动。
...
### Claude（推荐：stdio proxy 注入 token）
...
│   ├── server/           # 本地 daemon + loupe CLI
│   ├── claude-plugin/    # Claude 插件：mcp-proxy / commands / agents / hooks
...
| `build` | `tsc`（仅 `@loupe/claude-plugin`） | 编译 proxy 到 `dist/` |
